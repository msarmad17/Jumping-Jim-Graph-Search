...
bfs
{
    ...
    while q not empty
        de-q v
        en-q undisc neighbors of v, check if any of them is G
        if q size equals 1 and no neighbors in prev step, give up
        mark v exp
        mark v's neighbors as disc
}
*/
...
int main(int argc, char * argv[])
{
    ...
    for(int i = 0; i < r; i++)
    {
        for(int j = 0; j < c; j++)
        {
            ...
            (maze[i][j]).disc = 0;
            (maze[i][j]).exp = 0;
            //((maze[i][j]).route).erase( ((maze[i][j]).route).begin(), ( (maze[i][j]).route ).end() );
            (maze[i][j]).route = "\0";
        }
    }
    ...
    while(!trav.empty())
    {
        ...
        if((*temp).x - (*temp).val >= 0) //up
            if( !((maze[(*temp).x - (*temp).val][(*temp).y]).disc) )
            {
                ...
                //strcpy( (maze[(*temp).x - (*temp).val][(*temp).y]).ro, (*temp).ro);
                //(maze[(*temp).x - (*temp).val][(*temp).y]).ro[ sizeof((maze[(*temp).x - (*temp).val][(*temp).y]).ro) ] = 'N';
                trav.push( (maze[(*temp).x - (*temp).val][(*temp).y]) );
                //cout << (maze[(*temp).x - (*temp).val][(*temp).y]).route << endl;
            }
        ...
        if((*temp).y - (*temp).val >= 0) //left
            if( !((maze[(*temp).x][(*temp).y - (*temp).val]).disc) )
            {
                 ...
                 //cout << (maze[(*temp).x][(*temp).y - (*temp).val]).route << endl;
            }
